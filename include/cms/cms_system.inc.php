...
require_once 'cms/Cache.class.php';
...
if (!empty($sessid)) {
    ...
    if ($user_id) {
        ...
        $rs = $conn->Execute($sql);
        if ($rs->fields[0] <= 0) { //本地数据库不存在用户数据库记录，需要插入记录！
		    $user_infos['user_id'] = $user_id;
			$rec = $v->arraymem(0);
            $v = $rec->structmem('user_name');
           $user_infos['user_name'] = $v->scalarval();
			$v = $rec->structmem('password');
            $user_infos['password'] = $v->scalarval(); 
            $v = $rec->structmem('nickname');
            $user_infos['nickname'] = $v->scalarval();
			$v = $rec->structmem('email');
            $user_infos['email'] = $v->scalarval(); 
            $v = $rec->structmem('user_type');
           $user_infos['user_type'] = $v->scalarval();
			if (FALSE === $cms->do_after_register($user_infos)) {
           	$errmsg = ($cms->errmsg) ? $cms->errmsg : '遇到不可预知的错误。';
        	}
		}
        $max = $v->arraysize();
        if ($max > 0) {
	...
	*/   
        ...
        unset($_SESSION['CMS_ZZXX_USER_ID']);
        session_destroy();                   
    }   //end if
...
if (!empty($sessid) || TRUE === $do_logout) {
    $replace_url = 'http://'.$_SERVER['SERVER_NAME'].cms_filter(preg_replace('/(\?|&)(sessid=[^&]*|logout=[^&]*)/', '', $_SERVER["REQUEST_URI"]));
    ?>
