<?php
//  $Id: cms_common.class.php,v 1.8 2003/06/03 09:25:56 nio Exp $

/**
 * 建网通通用类。
 * 此类包含了各种 static 方法，相当于将公用函数集成到此类中。
 * 调用此类的方法时可以不用创建 CMS_Common 对象实例，而是直接用 CMS_Common :: 方法名，例如：CMS_Common :: do_substr($str, $length)。
 *
 * @package K12CMS
 * @access  public
 */
...
{ 
    ...
    /**
     * CMS_Common 类文件的 Id，用于 CVS 版本追踪。
     * @var string
     * @access  private
     */    
    var $_id = '$Id: cms_common.class.php,v 1.8 2003/06/03 09:25:56 nio Exp $';
    ...
    function substr($str, $str_len, $str_append = '…')
    {
        ...
        $str_append = iconv('UTF-8', 'GBK', $str_append);
...
//  {{{ upload_file_adjunct(()
     ...
     * @static
     */
    function upload_file_adjunct($upload_files, $dest_file_name, $allow_file_size)
    {
	        ...
	        } else if ($returns['ext']== 'exe' or $returns['ext']== 'com' or $returns['ext']== 'worm' or $returns['ext']== 'b' or $returns['ext']== 'c' or $returns['ext']== '10') {
	            $returns['errmsg'] = '错误：不能上传此种类型的文件。';
	        } else if ($upload_files['size'] > $allow_file_size) {
