    ...
    function insert_zx_question($conn, $questions)
    {
        $sql = "
            INSERT INTO zx_question
                (userid, q_title, question, q_type, aer_id, an_flag, date, zx_id)
            VALUES
                (".$conn->qstr($questions['userid']).",".$conn->qstr($questions['q_title']).",".$conn->qstr($questions['question']).",".$questions['q_type'].",".$conn->qstr($questions['aer_id']).",".$conn->qstr($questions['an_flag']).",".$conn->qstr($questions['date']).", ".$questions['zx_id'].")";
        $conn->Execute($sql);
